...
class ListaAtomica {
 ...
 public:
    ...
    void insertar(const T &valor) {
        ...
        while (true) {
            head = _cabeza.load();
            nuevoNodo->_siguiente = head;

            if (_cabeza.compare_exchange_weak(head, nuevoNodo)) {
                break;
            }
        }
    }

    bool estaVacia() const{ // Usada para debugging
        return (this->_cabeza == nullptr);
    }

    bool pertenece(const T& valor) { // Usado en tests
        Nodo* n = _cabeza.load();
        while (n != NULL) {
            if (n->_valor == valor) {
                return true;
